      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '订单编号' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search"
                      v-model="searchForm.orderid" placeholder="订单编号" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search"
                      v-model="searchForm.orderid" placeholder="订单编号" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.orderid" placeholder="订单编号"
                      clearable></el-input>
          </el-form-item>
          <el-form-item :label="contents.inputTitle == 1 ? '商品名称' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search"
                      v-model="searchForm.goodname" placeholder="商品名称" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search"
                      v-model="searchForm.goodname" placeholder="商品名称" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.goodname" placeholder="商品名称"
                      clearable></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            ...
            <download-excel v-if="isAuth('orders'+'/'+orderStatus,'导出')" class="export-excel-wrapper" :data="dataList"
                            :fields="json_fields" name="订单.xls">
              <!-- 导出excel -->
              <el-button
                  v-if="contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
                  type="danger"
                  icon="el-icon-download"
              >{{ contents.btnAdAllFont == 1 ? '导出' : '' }}
              </el-button>
              <el-button
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('orders'+'/'+orderStatus,'查看') && contents.tableBtnIcon == 0" type="success"
              ...
              <el-button v-if=" isAuth('orders'+'/'+orderStatus,'修改') && contents.tableBtnIcon == 0" type="primary"
                         size="mini" @click="addOrUpdateHandler(scope.row.id)">
                {{ contents.tableBtnFont == 1 ? '修改' : '' }}
              </el-button>

              <el-button
                  v-if="isAuth('orders'+'/'+orderStatus,'物流') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="primary" icon="el-icon-edit" size="mini" @click="logisticsUpdate(scope.row.id)">
                {{ contents.tableBtnFont == 1 ? '物流' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('orders'+'/'+orderStatus,'物流') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  type="primary" size="mini" @click="logisticsUpdate(scope.row.id)">
                {{ contents.tableBtnFont == 1 ? '物流' : '' }}<i class="el-icon-edit el-icon--right"/></el-button>
              <el-button v-if="isAuth('orders'+'/'+orderStatus,'物流') && contents.tableBtnIcon == 0" type="primary"
                         size="mini" @click="logisticsUpdate(scope.row.id)">{{
                  contents.tableBtnFont == 1 ? '物流' : ''
                }}
              </el-button>
              <el-button
                  v-if="isAuth('orders'+'/'+orderStatus,'发货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="primary" icon="el-icon-edit" size="mini" @click="updateHandler(scope.row)">
                {{ contents.tableBtnFont == 1 ? '发货' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('orders'+'/'+orderStatus,'发货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  type="primary" size="mini" @click="updateHandler(scope.row)">
                {{ contents.tableBtnFont == 1 ? '发货' : '' }}<i class="el-icon-edit el-icon--right"/></el-button>
              <el-button v-if="isAuth('orders'+'/'+orderStatus,'发货') && contents.tableBtnIcon == 0" type="primary"
                         size="mini" @click="updateHandler(scope.row)">{{ contents.tableBtnFont == 1 ? '发货' : '' }}
              </el-button>

              <el-button
                  v-if="isAuth('orders'+'/'+orderStatus,'确认收货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="primary" icon="el-icon-edit" size="mini" @click="updateHandler2(scope.row)">
                {{ contents.tableBtnFont == 1 ? '确认收货' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('orders'+'/'+orderStatus,'确认收货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  type="primary" size="mini" @click="updateHandler2(scope.row)">
                {{ contents.tableBtnFont == 1 ? '确认收货' : '' }}<i class="el-icon-edit el-icon--right"/></el-button>
              <el-button v-if="isAuth('orders'+'/'+orderStatus,'确认收货') && contents.tableBtnIcon == 0" type="primary"
                         size="mini" @click="updateHandler2(scope.row)">{{
                  contents.tableBtnFont == 1 ? '确认收货' : ''
                }}
              </el-button>
              ...
              <el-button v-if="isAuth('orders'+'/'+orderStatus,'删除') && contents.tableBtnIcon == 0" type="danger"
